<template>
  <div class="app-container">
    <el-page-header @back="$router.push('/group-messages')" title="返回列表" content="任务详情 / Task Details" />
        ...
        <el-col :xs="24" :sm="24" :md="15" :xl="16">
          <el-card class="detail-card mb-20" shadow="hover">
            ...
            <el-descriptions :column="isMobile ? 1 : 2" border size="large" class="custom-desc">
              ...
              <el-descriptions-item label="目标群数" :span="taskInfo?.status === 3 ? 1 : 2">
                {{ taskInfo?.targetCount || 0 }} 个
              </el-descriptions-item>
          ...
          <el-card class="detail-card mb-20" shadow="hover">
            ...
            <el-descriptions :column="1" border size="large" class="custom-desc">
              <el-descriptions-item label="目标客户群">
                <template v-if="taskInfo?.targetType === 0">
                  <el-tag type="info" effect="plain" class="custom-tag">全部客户群</el-tag>
                </template>
                <template v-else>
                  <el-tag type="primary" effect="plain" class="custom-tag mb-10">条件筛选客户群</el-tag>
                  <div class="filter-box" v-if="taskInfo?.targetCondition">
                    
                    <div class="filter-item">
                      <span class="f-label">执行群主：</span>
                      <span class="f-value">
                        <template v-if="!taskInfo.targetCondition.ownerUserids || taskInfo.targetCondition.ownerUserids.length === 0">
                          <el-tag type="info" effect="light" size="small">全部群主</el-tag>
                        </template>
                        <template v-else>
                          <div class="tags-wrapper">
                            <el-tag type="primary" effect="light" size="small" class="custom-tag" v-for="uid in taskInfo.targetCondition.ownerUserids" :key="uid">
                              {{ getUserName(uid) }}
                            </el-tag>
                          </div>
                        </template>
                    ...
                    <div class="filter-item" v-if="taskInfo.targetCondition.createTimeStart">
                      <span class="f-label">群创建时间：</span>
                      <span class="f-value">{{ taskInfo.targetCondition.createTimeStart }} <b>至</b> {{ taskInfo.targetCondition.createTimeEnd }}</span>
                    </div>

                    <div class="filter-item" v-if="taskInfo.targetCondition.groupNameKeywords?.length > 0">
                      <span class="f-label">群名关键字：</span>
                      <span class="f-value flex-wrap">
                        <span class="condition-badge">含任一关键字</span>
                        <el-tag size="small" type="success" effect="light" class="f-tag" v-for="t in taskInfo.targetCondition.groupNameKeywords" :key="t">
                          {{ t }}
                        </el-tag>
                      </span>
                    </div>
          ...
          <el-card v-if="taskInfo?.status === 2" class="detail-card mb-20" shadow="hover">
            <template #header>
              <div class="card-header">
                <div class="header-title">
                  ...
                  <span>群发下发进度 / Sending Results</span>
                </div>
            ...
            <el-table :data="sendResult?.groupList || []" border stripe v-loading="resultLoading">
              <el-table-column prop="groupName" label="客户群名称" min-width="200">
                <template #default="{ row }">
                   <div style="display: flex; align-items: center; gap: 8px;">
                    <el-icon color="#409eff"><Shop /></el-icon>
                    <span class="text-bold">{{ row.groupName || row.chatId }}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column prop="status" label="下发状态" width="150">
                <template #default="{ row }">
                  <el-tag :type="row.status === 1 ? 'success' : (row.status === 2 ? 'danger' : 'info')" effect="light">
                    {{ row.status === 1 ? '已发送' : (row.status === 2 ? '发送失败' : '未发送') }}
                  </el-tag>
                </template>
              </el-table-column>
          ...
          <div class="preview-affix">
            <div class="preview-integrated">
              ...
              <div class="preview-wrapper">
                 <MobilePreview :text="taskInfo?.content || ''" :attachments="taskInfo?.attachments || []" type="group" />
              </div>
            </div>
          </div>
        </el-col>
...
const fetchSendResult = async () => {
  ...
  try {
    const res = await getGroupMessageSendResult(id)
...
onMounted(async () => {
  ...
  try {
    ...
    const res = await getGroupMessage(id) as any
    if (res) {
      taskInfo.value = {
        ...res,
...
const getUserName = (userid: string) => {
  const user = staffList.value.find(u => u.userid === userid)
